...
import br.com.digitalhouse.marvelscomics.model.pojo.TextObject;
...
public class MainActivityViewModel extends AndroidViewModel {
    ...
    public void getAllComics(String format, String formatType, boolean noVariants,
                             String orderBy, String ts, String hash, String apiKey) {
        disposable.add(
                repository.getComics(format, formatType, noVariants, orderBy, ts, hash, apiKey)
                        .subscribeOn(Schedulers.io())
                        .observeOn(AndroidSchedulers.mainThread())
                        .doOnSubscribe(disposable1 -> loading.setValue(true))
                        .doOnTerminate(() -> loading.setValue(false))
                        .subscribe(result -> listaComics.setValue(result.getData().getResults()),
                                throwable -> {
